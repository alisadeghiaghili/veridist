...
class DocsToolchainContractTests(unittest.TestCase):
    ...
    def test_first_vertical_docs_are_callable_narrow_and_honest(self) -> None:
        ...
        self.assertIn("reads the CSV rows once", tutorial_page)
        self.assertIn("do not cap all memory used by Python", tutorial_page)
        self.assertIn("do not guarantee a particular processing speed", tutorial_page)
        ...
        self.assertIn("λ̂ = r / τ = 1 / 2 = 0.5", tutorial_page)
